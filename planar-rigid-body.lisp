
(defun name-attach-sym (name sym)
  (cond
    ((keywordp sym)
     (read-from-string (format nil ":~a~a" name sym)))
    ((numberp sym) sym)
    (t (read-from-string (format nil "~a~a" name sym)))))

(defun hinge-rod-state (name)
  "目前最好给定的name是无空格的，且无/或者|(或许可以用随机生成id代替); ~@
  后续编写这个函数和下列几个函数的生成函数(模型的注册函数)"
  (let ((theta (name-attach-sym name 'theta))
        (d-theta (name-attach-sym name 'd-theta))
        (omega (name-attach-sym name 'omega))
        (d-omega (name-attach-sym name 'd-omega))
        (alpha (name-attach-sym name 'alpha))
        (aOx (name-attach-sym name 'aOx))
        (aOy (name-attach-sym name 'aOy))
        (FAx (name-attach-sym name 'FAx))
        (FAy (name-attach-sym name 'FAy))
        (Ax (name-attach-sym name 'Ax))
        (Ay (name-attach-sym name 'Ay)))
    `((,theta ,d-theta) (,omega ,d-omega) ,alpha ,aOx ,aOy ,FAx ,FAy ,Ax ,Ay)))

(defun hinge-rod-deri (name)
  (let ((d-theta (name-attach-sym name 'd-theta))
        (omega (name-attach-sym name 'omega))
        (d-omega (name-attach-sym name 'd-omega))
        (alpha (name-attach-sym name 'alpha)))
    `((,d-theta (,omega) ,omega) (,d-omega (,alpha) ,alpha))))

(defun hinge-rod-frame-inner (para-list name)
  (let ((theta (name-attach-sym name 'theta))
        (alpha (name-attach-sym name 'alpha))
        (aOx (name-attach-sym name 'aOx))
        (aOy (name-attach-sym name 'aOy))
        (FAx (name-attach-sym name 'FAx))
        (FAy (name-attach-sym name 'FAy))
        (Ax (name-attach-sym name 'Ax))
        (Ay (name-attach-sym name 'Ay))
        ;; 参数
        (m (getf para-list :m))
        (l (getf para-list :l))
        (k (getf para-list :k)))
    `((,FAx () 0) (,FAy () 0) (,aOx () 0) (,aOy () 0)
      (,alpha (,theta ,FAx ,FAy ,aOx ,aOy)
              (/ (+ (* -1 (+ 9.8 ,aOy) (sin ,theta))
                    (* -1 ,aOx (cos ,theta))
                    (* ,k (/ ,FAx ,m) (cos ,theta))
                    (* ,k (/ ,FAy ,m) (sin ,theta)))
                 ,l))
      (,Ax (,theta) (* ,k ,l (sin ,theta)))
      (,Ay (,theta) (* -1 ,k ,l (cos ,theta))))))

(let* ((name "simple-rod")
       (state (hinge-rod-state name))
       (deri (hinge-rod-deri name))
       (frame-inner (hinge-rod-frame-inner '(:m 1.0 :l 0.25 :k 1.0) name)))
  (let* ((solver (solver-constructor:solver-create state deri frame-inner))
         (eval-solver (eval solver))
         (initial-cond '(:theta 0.3 :alpha 0 :omega 0 :aOx 0 :aOy 0 :FAx 0 :FAy 0))
         (initial-cond (mapcar #'(lambda (x) (name-attach-sym name x)) initial-cond))
         (result (funcall eval-solver initial-cond 0.01 100)))
    (dump-result t result)))

(defun convert (result)
  "对result计算结果进行转换"
  (mapcar #'(lambda (frame)
              `((:|line|
                  (:|x1| 0 :|y1| 0
                   :|x2| ,(or (getf frame :Ax) 0)
                   :|y2| ,(- (or (getf frame :Ay) 0))))))
          result))

(defun scale-trans-ami (scale dx dy ami)
  "对动画进行缩放和平移"
  (let* ((ami
           (mapcar
            #'(lambda (frame)
                (mapcar
                 #'(lambda (elem)
                     (list (car elem)
                           (mapcar #'(lambda (x) (if (numberp x) (* x scale) x))
                                   (cadr elem))))
                 frame))
            ami))
         (ami
           (mapcar
            #'(lambda (frame)
                (mapcar
                 #'(lambda (elem)
                     (let ((attrs (cadr elem)))
                       (setf (getf attrs :|x1|) (+ dx (getf attrs :|x1|)))
                       (setf (getf attrs :|x2|) (+ dx (getf attrs :|x2|)))
                       (setf (getf attrs :|y1|) (+ dy (getf attrs :|y1|)))
                       (setf (getf attrs :|y2|) (+ dy (getf attrs :|y2|)))
                       (list (car elem) attrs)))
                 frame))
            ami)))
    ami))
